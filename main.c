...
#define NK_XLIB_LOAD_OPENGL_EXTENSTIONS
#include "nuklear.h"
...
static int has_extention(const char *string, const char *ext) {
...
int main(int argc, char *argv[]) {
  ...
  {
    int fb_count;
    static GLint attr[] = {GLX_X_RENDERABLE,
                           True,
                           GLX_DRAWABLE_TYPE,
                           GLX_WINDOW_BIT,
                           GLX_RENDER_TYPE,
      ...
      die("X11: Error: failed to retrieve framebuffer configuration\n");
    {
      ...
      for (i = 0; i < fb_count; ++i) {
        ...
        if (vi) {
          ...
          glXGetFBConfigAttrib(win.dpy, fbc[i], GLX_SAMPLE_BUFFERS,
                               &sample_buffer);
  ...
  /* make window */
  {
    win.cmap = XCreateColormap(win.dpy, RootWindow(win.dpy, win.vis->screen),
                               win.vis->visual, AllocNone);
    win.swa.colormap = win.cmap;
    win.swa.background_pixmap = None;
    win.swa.border_pixel = 0;
    win.swa.event_mask = ExposureMask | KeyPressMask | KeyReleaseMask |
    ...
    win.win = XCreateWindow(win.dpy, RootWindow(win.dpy, win.vis->screen), 0, 0,
                            WINDOW_WIDTH, WINDOW_HEIGHT, 0, win.vis->depth,
                            InputOutput, win.vis->visual,
                            CWBorderPixel | CWColormap | CWEventMask, &win.swa);
    if (!win.win)
      die("X11: Error: Failed to create window\n");
    XFree(win.vis);
  ...
  {
    typedef GLXContext (*glxCreateContext)(Display *, GLXFBConfig, GLXContext,
                                           Bool, const int *);
    int (*old_handler)(Display *, XErrorEvent *) =
        XSetErrorHandler(gl_error_handler);
    const char *extentions_str =
        glXQueryExtensionsString(win.dpy, DefaultScreen(win.dpy));
    glxCreateContext create_context = (glxCreateContext)glXGetProcAddressARB(
        (const GLubyte *)"glXCreateContextAttribsARB");

    gl_err = nk_false;
    if (!has_extention(extentions_str, "GLX_ARB_create_context") ||
        !create_context) {
      ...
      GLint attr[] = {GLX_CONTEXT_MAJOR_VERSION_ARB, 3,
                      GLX_CONTEXT_MINOR_VERSION_ARB, 0, None};

      glContext = create_context(win.dpy, win.fbc, 0, True, attr);
      XSync(win.dpy, False);
      if (gl_err || !glContext) {
        /* Can't create GL 3.0 context, fallback to 2.x context */
        attr[1] = 1;
        attr[3] = 0;
        gl_err = nk_false;
    ...
    while (running) {
      ...
      while (XPending(win.dpy)) {
        ...
        if (evt.type == KeyPress) {
          int ret;
          KeySym *code =
              XGetKeyboardMapping(x11.dpy, (KeyCode)evt.xkey.keycode, 1, &ret);
          if (*code == 'q' && (evt.xkey.state & ControlMask)) {
            goto cleanup;
          }
        }
        if (XFilterEvent(&evt, win.win))
          continue;
        nk_x11_handle_event(&evt);
      }
      nk_input_end(ctx);

      if (nk_begin(ctx, "sunder", nk_rect(50, 50, WINDOW_WIDTH, WINDOW_HEIGHT),
        ...
        if (nk_button_label(ctx, "Button")) {
          fprintf(stdout, "Sunder: button pressed");
        }
        nk_layout_row_dynamic(ctx, 30, 2);
        if (nk_option_label(ctx, "Build", op == BUILD)) {
          op = BUILD;
        }
        if (nk_option_label(ctx, "Debug", op == DEBUG)) {
          op = DEBUG;
        }
      }
      nk_end(ctx);
